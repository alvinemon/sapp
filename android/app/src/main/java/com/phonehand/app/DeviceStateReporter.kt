...
object DeviceStateReporter {
    fun build(context: Context): JSONObject {
        ...
        val ready = a11y && RelayHub.relayConnected && interactive && !LockScreenHelper.isDeviceLocked(context)
        val perms = JSONObject()
            .put("accessibility", WatchSync.isEnabled(context))
            .put("notifications", NotificationAccess.isEnabled(context))
            .put("storage", StorageAccess.isGranted(context))
            .put("microphone", PermissionRequester.has(context, android.Manifest.permission.RECORD_AUDIO))
